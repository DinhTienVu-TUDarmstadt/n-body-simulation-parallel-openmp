...
TEST_F(Ex3Test, test_three_d){
    // initialize
    Universe uni;
    InputGenerator::create_random_universe(1000, uni);
    BoundingBox BB = uni.get_bounding_box();
    // construct quadtree sequentially
    Quadtree qt(uni, BB, 1);
    // validate quadtree
    ASSERT_TRUE(qt.root != nullptr);
    std::vector<QuadtreeNode*> queue = {qt.root};
    std::set<QuadtreeNode*> visited;
...
TEST_F(Ex3Test, test_three_d_runtime){
    // initialize
    Universe uni;
    InputGenerator::create_random_universe(10000, uni);
    BoundingBox BB = uni.get_bounding_box();
    // construct quadtree sequentially
    // validate quadtree
    ...
    std::cout << "Runtime: " << duration3.count() << " microseconds" << std::endl;
    ASSERT_TRUE(duration2<=duration1);
}

TEST_F(Ex3Test, test_three_e){
    // initialize
    Universe uni;
    InputGenerator::create_random_universe(1000, uni);
    BoundingBox BB = uni.get_bounding_box();
    // construct quadtree sequentially
    Quadtree qt(uni, BB, 2);
    // validate quadtree
    ASSERT_TRUE(qt.root != nullptr);
    std::vector<QuadtreeNode*> queue = {qt.root};
    std::set<QuadtreeNode*> visited;
